...
export function FieldsDesigner({ value, onChange }: { value: Field[]; onChange: (fields: Field[]) => void }) {
                ...
                {draft.map((f, idx) => (
                    <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 160px 100px 80px', gap: 8 }}>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                const v = [...draft];
                                v[idx] = { ...v[idx], label: e.target.value };
                                setDraft(v);
                            }}
                        />
                        <select
                            ...
                            onChange={(e) => {
                                const v = [...draft];
                                v[idx] = { ...v[idx], type: e.target.value as Field['type'] };
                                setDraft(v);
                            }}
                        >
                        ...
                        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                            <input
                                ...
                                onChange={(e) => {
                                    const v = [...draft];
                                    v[idx] = { ...v[idx], required: e.target.checked };
                                    setDraft(v);
                                }}
                            />
                            Required
