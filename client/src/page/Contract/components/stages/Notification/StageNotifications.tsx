...
const StageNotifications: React.FC = () => {
	...
	const [notificationSettings, setNotificationSettings] = useState<NotificationSettings>(
		(formData as any)?.notificationSettings || {
	...
	const handleFormSubmit = () => {
		// Optionally persist to store/server later
		console.log("Notification settings saved (pending persistence):", notificationSettings);
				...
				<div className={cx("mainGrid", { sidebarCollapsed })}>
					<div className={cx("card")}>
						<div className={cx("cardHeader")}>
							<div className={cx("tabNavigation")}>
								{[
									...
									<button key={tab.key} className={cx("tabButton", { active: activeTab === tab.key })} onClick={() => setActiveTab(tab.key as any)}>
						...
						<div className={cx("cardContent")}> 
							...
							{activeTab === "global" && (
								<GlobalNotificationSettings settings={notificationSettings.globalSettings} onSettingsChange={handleUpdateGlobalSettings} />
							)}
						</div>
					</div>
					...
					<div className={cx("footerBar")}>
						<div className={cx("footerActions")}>
							<Button outline medium onClick={() => goToStep(Math.max(1, currentStep - 1))}>
