...
const UserManagement = () => {
    ...
    const filters = useMemo(
        () => ({ search, role: roleFilter, departmentId, page, limit }),
        [search, roleFilter, departmentId, page, limit],
    );
                        ...
                        {users.map((u) => (
                            <tr key={u.id} style={{ borderTop: '1px solid #eee' }}>
                                ...
                                <td style={{ display: 'flex', gap: 8 }}>
                                    ...
                                    <button onClick={() => onDeactivate(u)} disabled={!u.isActive}>
                                        Vô hiệu hóa
                                    </button>
                                </td>
                            </tr>
                ...
                <div style={{ position: 'fixed', inset: 0, background: '#0004' }} onClick={() => setDrawerOpen(false)}>
                    ...
                    >
                        ...
                        <UserForm
                            ...
                            onSubmit={(data) =>
                                selected ? onUpdate(data as UpdateAdminUserDto) : onCreate(data as CreateAdminUserDto)
                            }
                        />
                    </div>
                </div>
        ...
        >
            ...
            <label>
                ...
                <input
                    ...
                    onChange={(e) =>
                        setForm({ ...form, departmentId: e.target.value ? Number(e.target.value) : undefined })
                    }
                />
            </label>
            <label>
            ...
            {rows.map((r, i) => (
                <div key={i} style={{ display: 'flex', gap: 8 }}>
                    <input
                        ...
                        value={r.roleId}
                        onChange={(e) =>
                            setRows(rows.map((x, idx) => (idx === i ? { ...x, roleId: e.target.value } : x)))
                        }
                    />
                    <select
                        value={r.scope}
                        onChange={(e) =>
                            setRows(rows.map((x, idx) => (idx === i ? { ...x, scope: e.target.value as any } : x)))
                        }
                    >
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            setRows(
                                rows.map((x, idx) =>
                                    idx === i
                                        ? { ...x, scopeId: e.target.value ? Number(e.target.value) : undefined }
                                        : x,
                                ),
                            )
                        }
