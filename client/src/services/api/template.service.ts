...
class TemplateService extends BaseService {
    ...
    async diff(id: string, versionId: string, targetVersionId: string): Promise<ApiResponse<{ changes: any[] }>> {
        return this.post<{ changes: any[] }>(`/contracts/templates/${id}/versions/${versionId}/diff`, {
            targetVersionId,
        } as any);
    }

    async rollback(id: string, versionId: string): Promise<ApiResponse<{ version_id: string }>> {
        return this.post<{ version_id: string }>(
            `/contracts/templates/${id}/versions/${versionId}/rollback`,
            {} as any,
        );
    }
