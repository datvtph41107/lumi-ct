...
const { slice, thunks, actions } = createApiSlice({
    ...
    extraReducers: (builder, thunks) => {
        const { login, getCurrentUser, logout, verifySession, refreshToken, updateActivity, getUserPermissions } = thunks;
        builder
...
export const { login, getCurrentUser, logout, verifySession, refreshToken, updateActivity, getUserPermissions } = thunks;
