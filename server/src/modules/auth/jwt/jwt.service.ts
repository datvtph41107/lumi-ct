...
export class TokenService {
    ...
    async getUserTokens(user: User, context: UserContext, sessionId: string): Promise<{ access_token: string; refresh_token: string }> {
        const jti = uuidv4();
    ...
    decode(token: string): any { return this.refreshJwtService.decode(token); }
    ...
    private async createUserSession(userId: number, sessionId: string, jti: string): Promise<void> {
        try {
            ...
            await repo.save(repo.create({ session_id: sessionId, user_id: userId, jti, created_at: new Date(), last_activity: new Date(), is_active: true } as any));
        } catch (error) {
    ...
    async cleanupExpiredTokens(): Promise<void> {
        try {
            ...
            await sessionRepo.createQueryBuilder().delete().where('is_active = false AND revoked_at < :date', { date: oldSessionDate }).execute();
    ...
    async getSessionStatistics(): Promise<{ totalActiveSessions: number; totalExpiredSessions: number; totalRevokedTokens: number; }> {
        const sessionRepo = this.db.getRepository(UserSession);
        const revokedTokenRepo = this.db.getRepository(RevokedToken);
        ...
        return { totalActiveSessions: activeSessions, totalExpiredSessions: expiredSessions, totalRevokedTokens: revokedTokens };
    }
}
