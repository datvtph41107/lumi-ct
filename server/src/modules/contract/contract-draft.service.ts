...
export class ContractDraftService {
	...
	async getDraft(id: string) {
		...
		if (!c) return null;
		const latestStage = await this.draftRepo.findOne({ where: { contract_id: id }, order: { created_at: 'DESC' as any } });
		return this.toClientDraftShape(c, latestStage || undefined);
	}
	...
	async deleteDraft(id: string, userId: number) {
		await this.contractRepo.update(
			{ id },
			{ deleted_at: new Date(), deleted_by: String(userId) },
		);
		return { ok: true };
	}
	...
	async saveStage(contractId: string, stage: string, payload: { data: any }, userId: number) {
		...
		await this.contractRepo.update(
			{ id: contractId },
			{ last_auto_save: new Date(), updated_by: String(userId) },
		);
		return { ok: true };
	}
