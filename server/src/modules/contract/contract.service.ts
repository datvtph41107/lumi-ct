...
export class ContractService {
    ...
    async create(dto: CreateContractDto, ctx: { userId: number }) { return this.createContract(dto, ctx.userId); }
    async softDelete(id: string, userId: number) { return this.deleteContract(id as any, userId); }

    async listContracts(query: any, userId: number) {
        const page = Number(query.page || 1); const limit = Number(query.limit || 10);
        const [data, total] = await this.contractRepository.findAndCount({ where: { deleted_at: null as any }, take: limit, skip: (page - 1) * limit, order: { id: 'DESC' as any } });
        return { data, total, page, limit };
    }

    async createContract(createDto: CreateContractDto, userId: number): Promise<Contract> {
        if (!(await this.authCoreService.canCreateContract(userId, createDto.contract_type))) throw new ForbiddenException('Không có quyền tạo hợp đồng');
        const now = new Date();
        ...
        const content = this.contentRepository.create({ contract_id: savedContract.id, mode: (createDto as any).mode || 'basic', basic_content: null, editor_content: null, uploaded_file: null } as any);
        await this.contentRepository.save(content);
        const version = this.versionRepository.create({ contract_id: savedContract.id, version_number: 1, content_snapshot: {}, edited_by: String(userId), edited_at: now } as any);
        await this.versionRepository.save(version);

        await this.auditLogService.create({ contract_id: savedContract.id, user_id: userId, action: 'CREATE_CONTRACT' });
    ...
    async getContract(id: string, userId: number): Promise<Contract> {
        ...
        const can = await this.authCoreService.canReadContract(userId, 0 as any, { ownerId: contract.created_by, status: contract.status, type: contract.contract_type });
    ...
    async updateContract(id: string, updateDto: any, userId: number): Promise<Contract> {
        const contract = await this.contractRepository.findOne({ where: { id } });
        if (!contract) throw new NotFoundException('Hợp đồng không tồn tại');
        const can = await this.authCoreService.canUpdateContract(userId, 0 as any, { ownerId: contract.created_by, status: contract.status, type: contract.contract_type });
    ...
    async deleteContract(id: number, userId: number): Promise<void> {
        ...
        if (!contract) throw new NotFoundException('Hợp đồng không tồn tại');
        const can = await this.authCoreService.canDeleteContract(userId, 0 as any, { ownerId: contract.created_by, status: contract.status });
    ...
    async exportDocx(id: string, userId: number) { return { fileUrl: `/exports/${id}.docx` }; }
    async generatePrintView(id: string, userId?: number) { return { html: `<html><body>Contract ${id}</body></html>` }; }

    async createNotification(id: string, dto: any, userId: number) {
        await this.notificationService.create({ type: dto.type, title: dto.title, message: dto.message, userId, data: id });
        return { success: true };
    }
    async listNotifications(id: string) { return this.notificationService.getNotificationsByContract(id); }
    async createReminder(id: string, dto: any, userId: number) { return this.notificationService.createReminder({ contract_id: id, user_id: userId, ...dto }); }
    async listReminders(id: string) { return this.notificationService.getRemindersByContract(id); }
}
