...
export class ContractService {
	...
	async listDrafts(query: any, userId: number) {
		...
		const qb = this.contractRepository
			...
			.skip((page - 1) * limit)
			.take(limit);
		if (query.search) qb.andWhere('(LOWER(c.name) LIKE :kw OR LOWER(c.contract_code) LIKE :kw)', { kw: `%${String(query.search).toLowerCase()}%` });
	...
	async createDraft(payload: { name: string; contract_code?: string; contract_type?: string; category?: string; priority?: any; mode: any; template_id?: string; initialData?: any }, userId: number) {
		...
		if (payload.initialData) {
			const draft = this.draftRepository.create({ contract_id: saved.id, stage: 'content_draft', data: payload.initialData, version: 1, created_by: userId } as any);
	...
	async saveStage(contractId: string, stage: string, payload: { data: any }, userId: number) {
		const draft = this.draftRepository.create({ contract_id: contractId, stage, data: payload.data, version: 1, created_by: userId } as any);
		await this.draftRepository.save(draft);
		await this.contractRepository.update({ id: contractId } as any, { last_auto_save: new Date(), updated_by: String(userId) } as any);
		return { ok: true } as any;
	}
	...
	async rollbackToVersion(contractId: string, versionId: string, userId: number): Promise<{ version_id: string }> {
		...
		const latest = await this.versionRepository.findOne({ where: { contract_id: contractId }, order: { version_number: 'DESC' as any } });
		const nextVersionNumber = ((latest?.version_number as any) || 0) + 1;
		const newVersion = this.versionRepository.create({ contract_id: contractId, version_number: nextVersionNumber, content_snapshot: snapshot, edited_by: String(userId), edited_at: new Date() } as any);
		const savedVersion = await this.versionRepository.save(newVersion);
		await this.auditLogService.create({ contract_id: contractId, user_id: userId, action: 'ROLLBACK_VERSION', meta: { from_version_id: versionId, to_version_number: nextVersionNumber }, description: `Rollback to version ${version.version_number}` });
