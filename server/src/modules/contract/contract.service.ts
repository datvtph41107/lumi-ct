...
export class ContractService {
	...
	async listContracts(query: any, userId: number) {
		const pagination = { page: Number(query.page || 1), limit: Number(query.limit || 10) } as any;
		const filters = { status: query.status, type: query.type, search: query.search } as any;
		return this.listContractsInternal(filters, pagination, userId);
	}

	private async listContractsInternal(
		...
		return { data, total: data.length, page: pagination.page || 1, limit: pagination.limit || 10 };
	}
	...
	async submitForReview(id: string, userId: number): Promise<Contract> {
	...
	async approveContract(id: string, userId: number, comment?: string): Promise<Contract> {
		const contract = await this.getContract(id, userId);
	...
	async rejectContract(id: string, userId: number, reason: string): Promise<Contract> {
		const contract = await this.getContract(id, userId);
	...
	async requestChanges(id: string, userId: number, changes: string): Promise<Contract> {
		const contract = await this.getContract(id, userId);
	...
	async createMilestone(contractId: number, milestoneData: any, userId: number): Promise<Milestone> {
		const contract = await this.getContract(String(contractId), userId);
	...
	async exportContract(id: string, format: string, userId: number): Promise<any> {
		const contract = await this.getContract(id, userId);
	...
	async exportDocx(id: string, userId?: number): Promise<{ filename: string; contentBase64: string; contentType: string }> {
		...
		try { await this.auditLogService.create({ contract_id: contract.id, user_id: userId, action: 'EXPORT_DOCX' }); } catch {}
		return { filename: `${contract.name || 'contract'}.txt`, contentBase64: Buffer.from(content, 'utf8').toString('base64'), contentType: 'text/plain' };
	}

	async generatePrintView(id: string, userId?: number): Promise<{ html: string }> {
		const contract = await this.contractRepository.findOne({ where: { id } });
		if (!contract) throw new NotFoundException('Hợp đồng không tồn tại');
		const html = `<!doctype html><html><head><meta charset="utf-8"><title>${contract.name || 'contract'}</title><style>body{font-family:Arial,Helvetica,sans-serif;padding:24px}</style></head><body><h1>${contract.name || 'Hợp đồng'}</h1><p>Mã hợp đồng: ${contract.contract_code || ''}</p></body></html>`;
		try { await this.auditLogService.create({ contract_id: contract.id, user_id: userId, action: 'PRINT_CONTRACT' }); } catch {}
		return { html };
	}
	...
	async createNotification(contractId: string, dto: any, userId: number) {
		return this.notificationService.createNotification({ ...dto, contract_id: contractId, user_id: userId });
	}
	...
	async createReminder(contractId: string, dto: any, userId: number) {
		return this.notificationService.createReminder({ ...dto, contract_id: contractId, user_id: userId });
	}
